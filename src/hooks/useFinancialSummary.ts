...
export function useFinancialSummary() {
  const {
    ...
    queryFn: async (): Promise<FinancialSummary> => {
      ...
      // Sum of monthly payment amounts, not total balances owed
      ...
      // Calculate upcoming recurring expenses (due in current month, from today onwards)
      const activeRecurringExpenses = expenses.filter(
        (e) => e.is_active && e.frequency !== 'one_time'
      );
      const upcomingRecurringExpenses = activeRecurringExpenses.filter(
        (expense) => {
          if (!expense.due_date || !expense.start_date) return false;
          ...
          if (todayStart < startDate) return false;
      ...
      const upcomingBillsCount =
        upcomingBills.length + upcomingRecurringExpenses.length;
      const upcomingBillsTotal =
        upcomingBills.reduce((sum, bill) => sum + bill.amount, 0) +
        upcomingRecurringExpenses.reduce(
